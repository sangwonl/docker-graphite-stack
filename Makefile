...
run:
	mkdir -p $(HOME)/data/pgdata $(HOME)/data/graphite $(HOME)/data/grafana/etc $(HOME)/data/grafana/log $(HOME)/data/grafana/lib
	...
	docker run -d --name grafana -v $(HOME)/data/grafana/etc:/etc/grafana -v $(HOME)/data/grafana/lib:/var/lib/grafana -v $(HOME)/data/grafana/log:/var/log/grafana --link graphiteweb:graphiteweb -p 3000:3000 -e "GF_SECURITY_ADMIN_PASSWORD=admin" grafana
